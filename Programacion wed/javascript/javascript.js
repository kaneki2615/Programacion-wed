...
// ========================================================
// GENERADOR DE CREDENCIALES DINÁMICAS (TERCERA PÁGINA)
// ========================================================
const formCredencial = document.getElementById('formCredencial');

if (formCredencial) {
    formCredencial.addEventListener('submit', (evento) => {
        // Evita que la página se recargue al presionar el botón de enviar
        evento.preventDefault();
        
        // 1. Extraer los valores que el usuario escribió en el formulario
        const nombreRecluta = document.getElementById('regNombre').value;
        const distritoAsignado = document.getElementById('regDistrito').value;
        const quinqueSeleccionado = document.getElementById('regQuinque').value;
        
        // 2. Insertar de manera dinámica los textos dentro del Carnet Virtual
        document.getElementById('cardNombre').textContent = nombreRecluta;
        document.getElementById('cardDistrito').textContent = distritoAsignado;
        document.getElementById('cardQuinque').textContent = quinqueSeleccionado;
        
        // 3. Manipular el DOM para ocultar la vista de espera y revelar el carnet
        const vistaPreviaVacia = document.getElementById('vistaPreviaVacia');
        const contenedorCredencial = document.getElementById('contenedorCredencial');
        
        if (vistaPreviaVacia && contenedorCredencial) {
            vistaPreviaVacia.classList.add('d-none'); // Oculta el mensaje gris
            contenedorCredencial.classList.remove('d-none'); // Muestra el carnet final
        }
        
        // Opcional: Si cuentas con la consola integrada de logs del paso anterior
        if (typeof registrarEvento === "function") {
            registrarEvento(`Nueva credencial militar expedida con éxito para: ${nombreRecluta}.`);
        }
    });
}